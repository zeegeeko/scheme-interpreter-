...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #evaluate operands by recursive call
...
def eval_all(expressions, env):
    ...
    #iterate and evaluate each expression in order until the last expression
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        "*** REPLACE THIS LINE ***"
        #check if the formals and vals are the same length
        if len(formals) != len(vals):
            raise SchemeError('Number of arguments do not match number of parameters.')

        #iterate throught the formals and vals and entering it into the frame's dictionary
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** REPLACE THIS LINE ***"
        #target or expression.first is a list with the lambda name as first element
        #define the lambda by target.first and construct lambda procedure
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    "*** REPLACE THIS LINE ***"
    #if no sub expressions ie (and) return True
    if expressions is nil:
        return True

    #iterate through expressions and evaluate to check if it returns T/F
    while expressions.second is not nil:
    ...
    #If none of the expressions are False then return last sub expression
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 13
...
def do_or_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    "*** REPLACE THIS LINE ***"
    #if no sub expressions ie (or) return False
    if expressions is nil:
        return False
    ...
    #If none of the expressions are True then evaluate False expression
    return scheme_eval(expressions.first, env, True)
    # END PROBLEM 13
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** REPLACE THIS LINE ***"
            #If no sub expression return predicate
            if clause.second is nil:
                return test
            else:
                #evaluate and return last expression
...
def make_let_frame(bindings, env):
    ...
    #Iterate through all the Pairs in the bindings
    while bindings is not nil:
        #Check that each binding is in proper form
        check_form(bindings.first, 2 ,2)
        #Check if the binding has a proper symbol. If not raise SchemeError
        if not scheme_symbolp(bindings.first.first):
            raise SchemeError("Not a valid symbol")
        #Add formal and corresponding value by calling Frame's define method
