...
;;; Q7 tests
(define a '(cons 4 (cons 3 nil)))
; expect a
a
; expect (cons 4 (cons 3 ()))
...
;;; Q8 Tests
(define a True)
(begin '(+ 2 3) (equal? a False) (equal? a True))
; expect True
...
((lambda (x) (cons x (cons (f 8 2) nil))) 3)
; expect (3 16)

(define (h x) (cons x (cons (f 8 2) nil)))
; expect h
(h 4)
; expect (4 16)
...
(begin (or True (/ 1 0))(and False (/ 1 0)))
; expect False
...
(define (odd lst)
  (cond
    ((null? lst) nil)
    (else (cons (first lst) (even (rest lst))))
  )
)
